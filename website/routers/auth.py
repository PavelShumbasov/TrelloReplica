...
@router.post("/sign_up")
async def sign_up(
    request: Request, user: UserAuth = Depends(UserAuth), db: Session = Depends(get_db)
):
    ...
    if email_exists:
        ...
        new_user = User(
            ...
            password=generate_password_hash(user.password1, method="sha256"),
        )
        db.add(new_user)
        db.commit()
        ...
        )  # Создание токена доступа Логин менеджером для авторизации пользователя
...
async def login(request: Request, data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not user:
        ...
        access_token = manager.create_access_token(
            data={"sub": username}, expires=token_time
        )
        resp = RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
        manager.set_cookie(resp, access_token)
    return resp
...
async def auth(request: Request, db: Session = Depends(get_db)):
    ...
    access_token = manager.create_access_token(
        data={"sub": username}, expires=timedelta(weeks=4)
    )
    resp = RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
    manager.set_cookie(resp, access_token)
...
async def edit(
    request: Request,
    user: UserAuth = Depends(UserAuth),
